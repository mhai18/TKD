...
@endsection
@section('APP-CONTENT')
    <form id="updateForm" class="card-content needs-validation" novalidate data-id="{{ $player->id }}">
        <input type="hidden" id="chapter_id" name="chapter_id" value="{{ auth()->user()->chapter->id ?? '' }}">
        <input type="hidden" id="coach_id" name="coach_id" value="{{ auth()->user()->chapter->coach_id ?? '' }}">
        <input type="hidden" id="user_type" name="user_type" value="Player">
                ...
                <div class="row align-items-center">
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <input type="text" class="form-control" name="extension_name" id="extension_name"
                            value="{{ $player->user->extension_name }}">
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <input type="tel" class="form-control" name="contact_number" id="contact_number"
                            value="{{ $player->user->contact_number }}">
                        ...
                        <select class="form-control choicesjs" name="civil_status" id="civil_status">
                            @foreach (\App\Enums\CivilStatus::cases() as $status)
                                <option value="{{ $status->value }}"
                                    {{ $player->civil_status === $status ? 'selected' : '' }}>
                                    {{ $status->value }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-control choicesjs" name="religion" id="religion">
                            @foreach (\App\Enums\Religion::cases() as $religion)
                                <option value="{{ $religion->value }}"
                                    {{ $player->religion === $religion ? 'selected' : '' }}>
                                    {{ $religion->value }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-control choicesjs" name="belt_level" id="belt_level">
                            @foreach (\App\Enums\BeltLevel::cases() as $belt)
                                <option value="{{ $belt->value }}"
                                    {{ $player->belt_level === $belt ? 'selected' : '' }}>
                                    {{ ucwords(str_replace('_', ' ', $belt->value)) }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-control" name="province_code" id="province_code">
                            ...
                            @foreach ($provinces as $province)
                                <option value="{{ $province->province_code }}"
                                    {{ $player->province_code == $province->province_code ? 'selected' : '' }}>
...
@section('APP-SCRIPT')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            const selectedMunicipalityCode = "{{ $player->municipality_code }}";
            const selectedBrgyCode = "{{ $player->brgy_code }}";
            const selectedProvinceCode = "{{ $player->province_code }}";
            ...
            $('#updateForm').submit(function(event) {
                ...
                $.ajax({
                    method: 'POST',
                    url: `/players/${form.dataset.id}`, // Assumes form has data-id="{{ $player->id }}"
                    data: formData,
                    dataType: 'JSON',
                    contentType: false,
                    processData: false,
