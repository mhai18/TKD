...
@section('APP-TITLE')
    {{ $selectedChapter ? $selectedChapter->chapter_name : 'No Chapter Selected' }} | Coach
@endsection
                ...
                @if ($chapters->isNotEmpty())
                    <div class="form-group mb-0">
                        <select id="chapterSelect" class="form-control chapter-select"
                            onchange="window.location.href='{{ route('coachChapter') }}?chapter_id='+this.value">
                ...
                @else
                    <div class="col-lg-4">
                        <div class="card card-block p-card">
                            <div class="profile-box">
                                <div class="profile-card rounded">
                                    <img src="{{ $selectedChapter->coach->getFirstMediaUrl('avatar', 'thumb') ?: asset('assets/images/user/1.jpg') }}"
                                        alt="profile-bg" class="avatar-100 rounded d-block mx-auto img-fluid mb-3">
                                    <h3 class="font-600 text-white text-center mb-0">
                                        {{ $selectedChapter->coach->full_name }}</h3>
                                    <p class="text-white text-center mb-5">Coach</p>
                                </div>
                                <div class="pro-content rounded">
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <p class="mb-0 eml">{{ $selectedChapter->coach->email }}</p>
                                    </div>
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="p-icon mr-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="text-primary" width="20"
                                        ...
                                        <p class="mb-0">{{ $selectedChapter->coach->contact_number }}</p>
                                    </div>
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="p-icon mr-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="text-primary" width="20"
                                        ...
                                        <p class="mb-0">BRGY. {{ $selectedChapter->full_address }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-8">
                        <div class="card card-block">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4 class="card-title mb-0">Chapter: <span
                                        class="text-primary">{{ $selectedChapter->chapter_name }}</span></h4>
                                <h4 class="card-title mb-0">Started: <span
                                        class="text-primary">{{ date('F j, Y', strtotime($selectedChapter->date_started)) }}</span>
                                </h4>
                            </div>
                            <div class="card-header">
                                <h4 class="card-title mb-0">Location: BRGY. {{ $selectedChapter->full_address }}</h4>
                            </div>
                            <div class="card-header">
                                <h4 class="card-title mb-0">List of Players</h4>
                            </div>
                            <div class="card-body scrollable-content">
                                <div class="row">
                                    @if ($selectedChapter->players->isEmpty())
                                        <div class="col-12 text-center alert alert-info shadow-sm rounded">
                                            <div class="py-5">
                                                <h4 class="mb-3 font-weight-bold text-primary">No Players Found</h4>
                                                <p class="mb-4 text-muted">It seems there are no players registered in this
                                                    chapter yet.
                                                    Please check back later or add new players to this chapter.</p>
                                            </div>
                                        </div>
                                    @else
                                        @foreach ($selectedChapter->players as $player)
